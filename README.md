...
- **105 MCP Tools** across 19 categories for end-to-end document intelligence
...
- **GPU Embeddings** via local nomic-embed-text-v1.5 (768-dim, CUDA only)
...
**Components:**
- **TypeScript MCP Server** -- 105 tools, Zod schema validation, provenance tracking
...
- **nomic-embed-text-v1.5** -- 768-dim embeddings, local GPU inference only
...
| NVIDIA GPU | CUDA-capable | Embedding generation (required) |
| CUDA Toolkit | >= 12.0 | GPU inference |
...
Apple Silicon Macs can use MPS (Metal Performance Shaders) instead of CUDA. Update your `.env`:
```bash
...
1. Install [NVIDIA CUDA Toolkit](https://developer.nvidia.com/cuda-downloads) for Windows
2. Install Python via the [official installer](https://www.python.org/downloads/) or conda
3. Install PyTorch with CUDA:
   ```powershell
   pip install torch --index-url https://download.pytorch.org/whl/cu124
   ```
4. Use PowerShell or Git Bash for all commands
...
DATALAB_TIMEOUT=300000                 # 5 minutes

# Embedding model (local GPU only)
...
Then restart your Claude Code session. All 105 tools will be available immediately.
...
|----------|-------|-------|
| [Database Management](#database-management) | 6 | create, list, select, stats, delete, bulk_delete |
...
| `ocr_db_bulk_delete` | Delete multiple databases at once |
