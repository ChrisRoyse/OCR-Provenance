...
function generateTextDocument(charTarget: number): string {
  ...
  while (charCount < charTarget) {
    ...
    for (let p = 0; p < parasPerSection && charCount < charTarget; p++) {
      ...
      const para = Array.from({ length: sentCount }, (_, i) => sentences[i % sentences.length]).join(' ');
      sections.push(para);
...
function generateDocWithTables(charTarget: number): string {
  ...
  while (charCount < charTarget) {
    ...
    const rows = ['| ID | Name | Value | Category | Status |', '|----|------|-------|----------|--------|'];
...
describe('Performance Benchmarks', () => {
  ...
  for (const size of sizes) {
    it(`chunks ${size / 1000}K char text document within performance budget`, () => {
      ...
      console.error(`[BENCHMARK] ${size / 1000}K chars: ${chunks.length} chunks in ${elapsed.toFixed(1)}ms (${throughput} chars/sec)`);
  ...
  it('extractPageOffsetsFromText handles 1000-page document', () => {
    ...
    console.error(`[BENCHMARK] extractPageOffsetsFromText 1000 pages: ${elapsed.toFixed(1)}ms, ${offsets.length} offsets`);
