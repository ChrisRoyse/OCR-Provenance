...
describe('autoMergeIntoKnowledgeGraph precondition checks', () => {
  ...
  it.skipIf(!sqliteVecAvailable)('knowledge_nodes table exists in schema', () => {
    ...
    const result = conn
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='knowledge_nodes'")
  ...
  it.skipIf(!sqliteVecAvailable)('no KG nodes means COUNT(*) returns 0', () => {
    ...
    const row = conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes').get() as {
      cnt: number;
    };
  ...
  it.skipIf(!sqliteVecAvailable)(
    'autoMerge response shape when KG exists has expected fields',
    () => {
