...
function insertSearchableDoc(
  db: DatabaseService, chunkText: string, fileName: string,
  ...
  db.insertProvenance({
    ...
    source_type: 'FILE', source_path: `/tmp/${fileName}`, source_id: null,
  ...
  db.insertDocument({
    id: docId, file_path: `/tmp/${fileName}`, file_name: fileName,
  ...
  db.insertOCRResult({
    ...
    parse_quality_score: 4.0, page_count: 1, cost_cents: 0,
...
describe('QW-5: Search Result Export (ocr_search_export)', () => {
  ...
  it.skipIf(!sqliteVecAvailable)('should create output directory if it does not exist', async () => {
    const db = DatabaseService.create(dbName, undefined, tempDir);
    state.currentDatabase = db;
    state.currentDatabaseName = dbName;
