...
describe('Entity Operations', () => {
  ...
  describe('insertEntity', () => {
    it.skipIf(!sqliteVecAvailable)('inserts an entity record', () => {
      ...
      const row = db.prepare('SELECT * FROM entities WHERE id = ?').get('ent-1') as Record<
        string,
        unknown
      >;
    ...
    it.skipIf(!sqliteVecAvailable)('stores metadata as JSON', () => {
      ...
      const row = db.prepare('SELECT metadata FROM entities WHERE id = ?').get('ent-meta') as {
        metadata: string;
      };
  ...
  describe('insertEntityMention', () => {
    it.skipIf(!sqliteVecAvailable)('inserts a mention record', () => {
      ...
      const row = db.prepare('SELECT * FROM entity_mentions WHERE id = ?').get('men-1') as Record<
        string,
        unknown
      >;
      expect(row).toBeDefined();
  ...
  describe('getEntitiesByDocument', () => {
    it.skipIf(!sqliteVecAvailable)(
      'returns entities for a document ordered by type and name',
      () => {
        const now = new Date().toISOString();
  ...
  describe('deleteEntitiesByDocument', () => {
    it.skipIf(!sqliteVecAvailable)('deletes entities and cascades to mentions', () => {
      ...
      const mentions = db
        .prepare('SELECT COUNT(*) as cnt FROM entity_mentions WHERE document_id = ?')
