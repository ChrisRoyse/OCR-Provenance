...
function insertDocWithOCR(
  db: DatabaseService, costCents: number, mode: string, completedAt: string, fileName?: string,
): string {
  const docId = uuidv4();
  const provId = uuidv4();
  ...
  db.insertProvenance({
    ...
    source_type: 'FILE', source_path: `/tmp/${fileName || docId}.pdf`, source_id: null,
  ...
  db.insertDocument({
    id: docId, file_path: `/tmp/${fileName || docId}.pdf`,
    file_name: fileName || `${docId}.pdf`,
    file_hash: fileHash, file_size: 1000, file_type: 'pdf', status: 'complete',
    page_count: 5, provenance_id: provId, error_message: null, ocr_completed_at: now,
  });
  db.insertProvenance({
  ...
  db.insertOCRResult({
    ...
    parse_quality_score: 4.0, page_count: 5,
    cost_cents: costCents, content_hash: textHash,
    processing_started_at: now, processing_completed_at: completedAt,
...
describe('QW-3: Cost Analytics Tool (ocr_cost_summary)', () => {
  ...
  it.skipIf(!sqliteVecAvailable)('should return zeros for empty database with group_by=total', async () => {
  ...
  it.skipIf(!sqliteVecAvailable)('should exclude zero-cost documents from grouped results', async () => {
    const db = DatabaseService.create(dbName, undefined, tempDir);
    state.currentDatabase = db;
    state.currentDatabaseName = dbName;
