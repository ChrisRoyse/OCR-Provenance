...
function insertTestGraphData(db: DatabaseService): {
  ...
  conn
    ...
    VALUES (?, ?, 'person', 'Alice', 'alice', 0.9, ?, ?)`
    )
    .run(entityId1, docId, entityProvId, now);
  conn
    .prepare(
      `INSERT INTO entities (id, document_id, entity_type, raw_text, normalized_text, confidence, provenance_id, created_at)
    VALUES (?, ?, 'organization', 'Acme Corp', 'acme corp', 0.85, ?, ?)`
    )
    .run(entityId2, docId, entityProvId, now);
  ...
  conn
    ...
    VALUES (?, 'person', 'Alice', 'alice', NULL, 1, 1, 0.9, NULL, ?, ?, ?)`
    )
    .run(nodeId1, provId, now, now);
  conn
    .prepare(
      `INSERT INTO knowledge_nodes (id, entity_type, canonical_name, normalized_name,
    aliases, document_count, mention_count, avg_confidence, metadata, provenance_id, created_at, updated_at)
    VALUES (?, 'organization', 'Acme Corp', 'acme corp', NULL, 1, 1, 0.85, NULL, ?, ?, ?)`
    )
    .run(nodeId2, provId, now, now);
  ...
  conn
    ...
    VALUES (?, ?, ?, 'co_mentioned', 1.0, 1, ?, NULL, ?, ?)`
    )
    .run(edgeId, srcId, tgtId, JSON.stringify([docId]), provId, now);
  ...
  conn
    ...
    .run(uuidv4(), nodeId1, entityId1, docId, now);
  conn
    .prepare(
      `INSERT INTO node_entity_links (id, node_id, entity_id, document_id, similarity_score, created_at)
    VALUES (?, ?, ?, ?, 1.0, ?)`
    )
    .run(uuidv4(), nodeId2, entityId2, docId, now);
...
describe.skipIf(!sqliteVecAvailable)('handleKnowledgeGraphScanContradictions', () => {
  ...
  it('detects conflicting relationships', async () => {
    ...
    dbService.insertProvenance({
      ...
      root_document_id: docProvId,
      location: null,
      content_hash: computeHash('scan-kg'),
      input_hash: null,
      file_hash: null,
      processor: 'test',
      processor_version: '1.0.0',
      ...
      parent_ids: JSON.stringify([docProvId]),
      chain_depth: 2,
      chain_path: '["DOCUMENT","KNOWLEDGE_GRAPH"]',
    });
    ...
    conn
      ...
      VALUES (?, 'person', 'Bob', 'bob', 1, 1, 0.9, ?, ?, ?)`
      )
      .run(personId, provId, now, now);
    conn
      .prepare(
        `INSERT INTO knowledge_nodes (id, entity_type, canonical_name, normalized_name,
      document_count, mention_count, avg_confidence, provenance_id, created_at, updated_at)
      VALUES (?, 'organization', 'CompanyA', 'companya', 1, 1, 0.8, ?, ?, ?)`
      )
      .run(org1Id, provId, now, now);
    conn
      .prepare(
        `INSERT INTO knowledge_nodes (id, entity_type, canonical_name, normalized_name,
      document_count, mention_count, avg_confidence, provenance_id, created_at, updated_at)
      VALUES (?, 'organization', 'CompanyB', 'companyb', 1, 1, 0.8, ?, ?, ?)`
      )
      .run(org2Id, provId, now, now);
    ...
    conn
      ...
      VALUES (?, ?, ?, 'works_at', 1.0, 1, ?, ?, ?)`
      )
      .run(uuidv4(), s1, t1, JSON.stringify([docId]), provId, now);
    conn
      .prepare(
        `INSERT INTO knowledge_edges (id, source_node_id, target_node_id, relationship_type,
      weight, evidence_count, document_ids, provenance_id, created_at)
      ...
      .run(uuidv4(), s2, t2, JSON.stringify([docId]), provId, now);
  ...
  it('detects duplicate nodes by name similarity', async () => {
    ...
    dbService.insertProvenance({
      ...
      root_document_id: docProvId,
      location: null,
      content_hash: computeHash('dup-kg'),
      input_hash: null,
      file_hash: null,
      processor: 'test',
      processor_version: '1.0.0',
    ...
    conn
      .prepare(
        `INSERT INTO knowledge_nodes (id, entity_type, canonical_name, normalized_name,
      document_count, mention_count, avg_confidence, provenance_id, created_at, updated_at)
      VALUES (?, 'person', 'Jonathan Smith', 'jonathan smith', 1, 1, 0.9, ?, ?, ?)`
      )
      .run(uuidv4(), provId, now, now);
    conn
      .prepare(
        `INSERT INTO knowledge_nodes (id, entity_type, canonical_name, normalized_name,
      document_count, mention_count, avg_confidence, provenance_id, created_at, updated_at)
      VALUES (?, 'person', 'Jonathon Smith', 'jonathon smith', 1, 1, 0.9, ?, ?, ?)`
      )
      .run(uuidv4(), provId, now, now);

    const handler = knowledgeGraphTools['ocr_knowledge_graph_scan_contradictions'].handler;
    const response = await handler({ scan_types: ['duplicate_nodes'], similarity_threshold: 0.8 });
    const result = parseResponse(response);
...
describe.skipIf(!sqliteVecAvailable)('handleKnowledgeGraphEntityImport', () => {
  ...
  it('stores external_links when not dry_run', async () => {
    ...
    const node = conn
      ...
      .get(personNodeId) as { metadata: string | null } | undefined;
