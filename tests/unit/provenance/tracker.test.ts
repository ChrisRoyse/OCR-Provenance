...
describe('ProvenanceTracker', () => {
  ...
  describe('createProvenance()', () => {
    it.skipIf(!sqliteVecAvailable)('creates DOCUMENT provenance with chain_depth=0 and empty parent_ids', () => {
      ...
      console.log(`  - chain_depth: ${row.chain_depth} (expected: 0) ${row.chain_depth === 0 ? '✓' : '✗'}`);
      console.log(`  - parent_ids: ${row.parent_ids} (expected: []) ${row.parent_ids === '[]' ? '✓' : '✗'}`);
      console.log(`  - root_document_id: ${row.root_document_id} (expected: ${id}) ${row.root_document_id === id ? '✓' : '✗'}`);
    ...
    it.skipIf(!sqliteVecAvailable)('creates OCR_RESULT provenance with chain_depth=1 and [docId] in parent_ids', () => {
      ...
      console.log(`  - chain_depth: ${row.chain_depth} (expected: 1) ${row.chain_depth === 1 ? '✓' : '✗'}`);
      console.log(`  - parent_ids: ${row.parent_ids} (expected: ["${docId}"]) ${row.parent_ids === JSON.stringify([docId]) ? '✓' : '✗'}`);
      console.log(`  - parent_id: ${row.parent_id} (expected: ${docId}) ${row.parent_id === docId ? '✓' : '✗'}`);
    ...
    it.skipIf(!sqliteVecAvailable)('creates CHUNK provenance with chain_depth=2 and [docId, ocrId] in parent_ids', () => {
      ...
      console.log(`  - chain_depth: ${row.chain_depth} (expected: 2) ${row.chain_depth === 2 ? '✓' : '✗'}`);
      console.log(`  - parent_ids: ${row.parent_ids} (expected: ["${docId}","${ocrId}"]) ${row.parent_ids === JSON.stringify([docId, ocrId]) ? '✓' : '✗'}`);
    ...
    it.skipIf(!sqliteVecAvailable)('creates EMBEDDING provenance with chain_depth=3 and [docId, ocrId, chunkId] in parent_ids', () => {
      // Create full chain
      const docId = tracker.createProvenance({
        type: ProvenanceType.DOCUMENT,
        source_type: 'FILE',
      ...
      console.log(`  - chain_depth: ${row.chain_depth} (expected: 3) ${row.chain_depth === 3 ? '✓' : '✗'}`);
      ...
      expect(JSON.parse(row.chain_path)).toEqual(['DOCUMENT', 'OCR_RESULT', 'CHUNK', 'EMBEDDING']);
      expect(row.root_document_id).toBe(docId);
    });
    ...
    it.skipIf(!sqliteVecAvailable)('throws CHAIN_BROKEN when source_id does not exist', () => {
      ...
      expect(() => tracker.createProvenance({
        type: ProvenanceType.OCR_RESULT,
        source_type: 'OCR',
  ...
  describe('getProvenanceChain()', () => {
    it.skipIf(!sqliteVecAvailable)('returns complete 4-depth chain from EMBEDDING to DOCUMENT', () => {
      // Build full chain
      const docId = tracker.createProvenance({
        type: ProvenanceType.DOCUMENT,
        source_type: 'FILE',
      ...
      console.log(`  - current.id: ${chain.current.id} (expected: ${embId}) ${chain.current.id === embId ? '✓' : '✗'}`);
      console.log(`  - ancestors.length: ${chain.ancestors.length} (expected: 3) ${chain.ancestors.length === 3 ? '✓' : '✗'}`);
      console.log(`  - ancestors[0].id: ${chain.ancestors[0]?.id} (expected: ${chunkId}) ${chain.ancestors[0]?.id === chunkId ? '✓' : '✗'}`);
      console.log(`  - ancestors[1].id: ${chain.ancestors[1]?.id} (expected: ${ocrId}) ${chain.ancestors[1]?.id === ocrId ? '✓' : '✗'}`);
      console.log(`  - ancestors[2].id: ${chain.ancestors[2]?.id} (expected: ${docId}) ${chain.ancestors[2]?.id === docId ? '✓' : '✗'}`);
      console.log(`  - root.id: ${chain.root.id} (expected: ${docId}) ${chain.root.id === docId ? '✓' : '✗'}`);
      console.log(`  - isComplete: ${chain.isComplete} (expected: true) ${chain.isComplete ? '✓' : '✗'}`);
  ...
  describe('getProvenanceByRootDocument()', () => {
    it.skipIf(!sqliteVecAvailable)('returns all provenance for a document ordered by chain_depth', () => {
      const docId = tracker.createProvenance({
        type: ProvenanceType.DOCUMENT,
        source_type: 'FILE',
        root_document_id: '',
      ...
      console.log(`  - total records: ${records.length} (expected: 3) ${records.length === 3 ? '✓' : '✗'}`);
      console.log(`  - records[0].chain_depth: ${records[0].chain_depth} (expected: 0) ${records[0].chain_depth === 0 ? '✓' : '✗'}`);
      console.log(`  - records[1].chain_depth: ${records[1].chain_depth} (expected: 1) ${records[1].chain_depth === 1 ? '✓' : '✗'}`);
      console.log(`  - records[2].chain_depth: ${records[2].chain_depth} (expected: 2) ${records[2].chain_depth === 2 ? '✓' : '✗'}`);
  ...
  describe('Edge Cases', () => {
    ...
    it.skipIf(!sqliteVecAvailable)('correctly builds parent_ids for deep chain', () => {
      ...
      const docRow = rawDb.prepare('SELECT parent_ids FROM provenance WHERE id = ?').get(docId) as { parent_ids: string };
      const ocrRow = rawDb.prepare('SELECT parent_ids FROM provenance WHERE id = ?').get(ocrId) as { parent_ids: string };
      const chunkRow = rawDb.prepare('SELECT parent_ids FROM provenance WHERE id = ?').get(chunkId) as { parent_ids: string };
      const embRow = rawDb.prepare('SELECT parent_ids FROM provenance WHERE id = ?').get(embId) as { parent_ids: string };
  ...
  describe('chain_path verification', () => {
    it.skipIf(!sqliteVecAvailable)('sets correct chain_path for each type', () => {
      ...
      const docRow = rawDb.prepare('SELECT chain_path FROM provenance WHERE id = ?').get(docId) as { chain_path: string };
      const ocrRow = rawDb.prepare('SELECT chain_path FROM provenance WHERE id = ?').get(ocrId) as { chain_path: string };
      const chunkRow = rawDb.prepare('SELECT chain_path FROM provenance WHERE id = ?').get(chunkId) as { chain_path: string };
      const embRow = rawDb.prepare('SELECT chain_path FROM provenance WHERE id = ?').get(embId) as { chain_path: string };
      ...
      expect(JSON.parse(embRow.chain_path)).toEqual(['DOCUMENT', 'OCR_RESULT', 'CHUNK', 'EMBEDDING']);
    });
  });
});
