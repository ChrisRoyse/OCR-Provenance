...
describe('GeminiClient Context Caching (AI-1)', () => {
  ...
  describe('generateWithCache', () => {
    ...
    it('should throw for expired cache', async () => {
      ...
      await expect(
        client.generateWithCache(cacheId, 'describe this', fakeFile)
      ).rejects.toThrow('Cache expired');
    });
  });
...
describe('GeminiClient Batch API (AI-2)', () => {
  ...
  describe('batchAnalyzeImages', () => {
    ...
    it('should call onProgress callback for each item', async () => {
      ...
      const mockAnalyze = vi.spyOn(client, 'analyzeImage').mockRejectedValue(
    ...
    it('should capture errors per item without failing entire batch', async () => {
      const mockAnalyze = vi.spyOn(client, 'analyzeImage')
        .mockResolvedValueOnce({
          text: 'description',
          usage: { inputTokens: 10, outputTokens: 5, cachedTokens: 0, thinkingTokens: 0, totalTokens: 15 },
          model: 'test-model',
