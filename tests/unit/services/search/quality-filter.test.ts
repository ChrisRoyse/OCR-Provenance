...
function insertDocWithQuality(
  db: DatabaseService, qualityScore: number | null, chunkText: string,
  ...
  db.insertDocument({
    ...
    file_hash: fileHash, file_size: 1000, file_type: 'pdf', status: 'complete',
  ...
  db.insertProvenance({
    ...
    processing_params: {}, processing_duration_ms: null, processing_quality_score: qualityScore,
  ...
  db.insertOCRResult({
    ...
    extracted_text: chunkText, text_length: chunkText.length,
    datalab_request_id: `test-${uuidv4()}`, datalab_mode: 'balanced',
    parse_quality_score: qualityScore, page_count: 1, cost_cents: 0,
  ...
  db.insertProvenance({
    ...
    root_document_id: provId, location: null, content_hash: textHash,
    input_hash: null, file_hash: null, processor: 'chunker', processor_version: '1.0.0',
    processing_params: {}, processing_duration_ms: null, processing_quality_score: null,
    parent_id: ocrProvId, parent_ids: JSON.stringify([provId, ocrProvId]),
    chain_depth: 2, chain_path: '["DOCUMENT", "OCR_RESULT", "CHUNK"]',
  });
  db.insertChunk({
    id: chunkId, document_id: docId, ocr_result_id: ocrResultId,
    text: chunkText, text_hash: textHash, chunk_index: 0,
    character_start: 0, character_end: chunkText.length,
...
describe('QW-2: Quality-Filtered Search', () => {
  ...
  it.skipIf(!sqliteVecAvailable)('should return all results when min_quality_score is not set', async () => {
  ...
  it.skipIf(!sqliteVecAvailable)('should filter out low-quality documents with min_quality_score', async () => {
    const db = DatabaseService.create(dbName, undefined, tempDir);
    state.currentDatabase = db;
    state.currentDatabaseName = dbName;

    insertDocWithQuality(db, 1.0, 'low quality document about bananas');
    const { docId: highDoc } = insertDocWithQuality(db, 4.5, 'high quality document about bananas');
  ...
  it.skipIf(!sqliteVecAvailable)('should return empty results when no documents meet quality threshold', async () => {
    const db = DatabaseService.create(dbName, undefined, tempDir);
    state.currentDatabase = db;
    state.currentDatabaseName = dbName;
  ...
  it.skipIf(!sqliteVecAvailable)('should handle quality score at exact boundary', async () => {
    ...
    const { docId: exactDoc } = insertDocWithQuality(db, 3.0, 'boundary quality document about grapes');
  ...
  it.skipIf(!sqliteVecAvailable)('should work with min_quality_score of 0 (returns all)', async () => {
    const db = DatabaseService.create(dbName, undefined, tempDir);
    state.currentDatabase = db;
    state.currentDatabaseName = dbName;
  ...
  it.skipIf(!sqliteVecAvailable)('should combine min_quality_score with document_filter', async () => {
    const db = DatabaseService.create(dbName, undefined, tempDir);
    state.currentDatabase = db;
    state.currentDatabaseName = dbName;
    ...
    const response = await handleSearch({
      query: 'cherry', limit: 10, min_quality_score: 3.0, document_filter: [doc1],
    });
    const parsed = parseResponse(response);
    expect(parsed.success).toBe(true);
    expect(parsed.data!.total).toBe(1);
  ...
  it.skipIf(!sqliteVecAvailable)('should handle documents with no OCR quality score (null)', async () => {
    const db = DatabaseService.create(dbName, undefined, tempDir);
    state.currentDatabase = db;
    state.currentDatabaseName = dbName;
