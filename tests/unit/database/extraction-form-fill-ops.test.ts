...
describe('Extraction and Form Fill Operations', () => {
   ...
   */
  function createDocumentChain(): { docId: string; docProvId: string; ocrId: string; ocrProvId: string } {
  ...
  describe('Extraction operations', () => {
    ...
    it.skipIf(!sqliteVecAvailable)('retrieves multiple extractions ordered by created_at DESC', () => {
      const { docId, docProvId, ocrId, ocrProvId } = createDocumentChain();
    ...
    it.skipIf(!sqliteVecAvailable)('extractions are removed when parent document is deleted', () => {
      const { docId, docProvId, ocrId, ocrProvId } = createDocumentChain();
  ...
  describe('Form fill operations', () => {
    it.skipIf(!sqliteVecAvailable)('inserts and retrieves form fill by ID', () => {
      ...
      expect(ff!.field_data_json).toBe('{"name": {"value": "John Doe"}, "email": {"value": "john@example.com"}}');
  ...
  describe('Document metadata update', () => {
    ...
    it.skipIf(!sqliteVecAvailable)('uses COALESCE to preserve existing values when updating partially', () => {
      const { docId } = createDocumentChain();
