...
import { detectEntityBoundaryIssues } from '../../src/services/chunking/chunker.js';
...
function insertTestEntity(
  ...
  `
    ...
    .run(
      ...
      pageNumber,
      charStart,
      charEnd ?? (charStart !== null ? charStart + rawText.length : null),
      rawText,
      now
    );
...
function insertKGNode(
  ...
  const resolvedDocId =
    documentId ??
    (
      conn.prepare('SELECT document_id FROM entities WHERE id = ?').get(entityId) as
        | { document_id: string }
        | undefined
    )?.document_id ??
    '';
...
describe('GAP-D4: computeExtractionQualityScore', () => {
  it('returns score between 0 and 1 for normal inputs', () => {
    ...
    expect(result.metrics, 'metrics should include entity_density').toHaveProperty(
      'entity_density'
    );
    expect(result.metrics, 'metrics should include type_diversity').toHaveProperty(
      'type_diversity'
    );
  ...
  it('generates recommendations for low coverage metrics', () => {
    ...
    const hasRecommendation = result.recommendations.some(
      (r) =>
        r.includes('density') ||
        r.includes('diversity') ||
        r.includes('noise') ||
        r.includes('coverage')
    );
...
describe('GAP-D1: detectEntityBoundaryIssues', () => {
  it('detects entity spanning across chunk boundary', () => {
    ...
    const entityMentions = [{ char_start: 95, char_end: 105, raw_text: 'split_entity' }];
  ...
  it('returns 0 when entity does not cross boundary', () => {
    ...
    const entityMentions = [{ char_start: 10, char_end: 50, raw_text: 'entity_within_chunk' }];
  ...
  it('handles null char_start/char_end gracefully', () => {
    ...
    const entityMentions = [
      {
        char_start: null as unknown as number,
        char_end: null as unknown as number,
        raw_text: 'null_positions',
      },
    ];
...
describe.skipIf(!sqliteVecAvailable)('GAP-C4: Document get entity summary', () => {
  ...
  beforeEach(() => {
    ...
    insertTestEntity(conn, doc1.docId, 'person', 'Jane Doe', 'jane doe', 0.9);
  ...
  it('returns entity_summary when include_entity_summary=true', async () => {
    ...
    const byType = summary.by_type as Array<{
      entity_type: string;
      count: number;
...
describe.skipIf(!sqliteVecAvailable)('GAP-D6: Database stats KG health', () => {
  ...
  it('returns kg_health with all required metrics', async () => {
    ...
    const _edgeId = insertKGEdge(conn, n1.nodeId, n2.nodeId, 'works_at', 0.9, 1);
  ...
  it('returns zero defaults when KG tables are empty', async () => {
    ...
    const _doc = insertCompleteDocChain(dbService, 'empty.pdf', '/test/empty.pdf', 'No KG data');
  ...
  it('correctly counts orphaned nodes (nodes with no edges)', async () => {
    ...
    const _n1 = insertKGNode(conn, e1.entityId, 'Orphan', 'person');
    // _n1 has no edges, so it should be orphaned
    ...
    expect(kgHealth.orphaned_nodes, 'should match actual orphan count from database').toBe(
      orphanCount
    );
...
describe.skipIf(!sqliteVecAvailable)('GAP-A1: Comparison contradictions -> KG edges', () => {
  ...
  it('increments contradiction_count on KG edges when comparison detects contradictions', async () => {
    ...
    conn
      .prepare(
        'INSERT INTO node_entity_links (id, node_id, entity_id, document_id, similarity_score, resolution_method, created_at) VALUES (?, ?, ?, ?, 1.0, ?, ?)'
      )
      .run(
        uuidv4(),
        n1Person.nodeId,
    ...
    const beforeAcme = conn
      .prepare('SELECT contradiction_count FROM knowledge_edges WHERE id = ?')
      .get(edgeAcme) as { contradiction_count: number };
    const beforeBeta = conn
      .prepare('SELECT contradiction_count FROM knowledge_edges WHERE id = ?')
      .get(edgeBeta) as { contradiction_count: number };
    ...
    if (kgEdgesUpdated > 0) {
      ...
      const afterAcme = conn
        .prepare('SELECT contradiction_count FROM knowledge_edges WHERE id = ?')
        .get(edgeAcme) as { contradiction_count: number };
      const afterBeta = conn
        .prepare('SELECT contradiction_count FROM knowledge_edges WHERE id = ?')
        .get(edgeBeta) as { contradiction_count: number };

      // At least one should have been incremented
      const totalIncrements =
        afterAcme.contradiction_count -
        beforeAcme.contradiction_count +
        (afterBeta.contradiction_count - beforeBeta.contradiction_count);
      expect(
        totalIncrements,
        'total contradiction_count increments should match kg_edges_updated'
      ).toBe(kgEdgesUpdated);
    }
  });

  it('comparison still succeeds when KG tables have no relevant edges', async () => {
    const _conn = dbService.getConnection();
...
describe.skipIf(!sqliteVecAvailable)('GAP-B1: Image entity filter', () => {
  ...
  beforeEach(() => {
    ...
    insertTestEntity(conn, doc1.docId, 'organization', 'Acme Corp', 'acme corp', 0.9, 2);
...
describe('GAP-C2: VLM canonical name COALESCE query', () => {
  it('vlm.ts contains COALESCE query using KG canonical_name', async () => {
    ...
    const vlmSource = fs.readFileSync(join(process.cwd(), 'src', 'tools', 'vlm.ts'), 'utf-8');
...
describe.skipIf(!sqliteVecAvailable)('GAP-D6 Extended: Orphan and temporal edge counting', () => {
  ...
  it('entity_link_coverage = linked_entities / total_entities', async () => {
    const conn = dbService.getConnection();
    const doc = insertCompleteDocChain(
      dbService,
      'coverage.pdf',
      '/test/coverage.pdf',
      'Coverage test'
    ...
    const _e3 = insertTestEntity(conn, doc.docId, 'person', 'Unlinked', 'unlinked');
...
describe.skipIf(!sqliteVecAvailable)('GAP-C3 Extended: include_entity_counts', () => {
  ...
  it('returns entity_mention_count and kg_node_count when include_entity_counts=true', async () => {
    ...
    const _e2 = insertTestEntity(conn, doc.docId, 'organization', 'Org1', 'org1');
...
describe('Schema Verification for GAP-dependent columns', () => {
  it('knowledge_edges has contradiction_count column', () => {
    ...
    try {
      ...
      const columns = db.prepare('PRAGMA table_info(knowledge_edges)').all() as Array<{
        name: string;
      }>;
  ...
  it('entity_extraction_segments table exists with required columns', () => {
    ...
    try {
      ...
      const columns = db.prepare('PRAGMA table_info(entity_extraction_segments)').all() as Array<{
        name: string;
      }>;
      const colNames = columns.map((c) => c.name);
  ...
  it('node_entity_links table exists for entity-to-KG mapping', () => {
    ...
    try {
      ...
      const columns = db.prepare('PRAGMA table_info(node_entity_links)').all() as Array<{
        name: string;
      }>;
      const colNames = columns.map((c) => c.name);
