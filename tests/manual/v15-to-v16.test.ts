...
describe('Migration v15 to v16 (Knowledge Graph)', () => {
  ...
  function createV15Schema(): void {
    ...
    db.exec(
      `CREATE TABLE fts_index_metadata (id INTEGER PRIMARY KEY, last_rebuild_at TEXT, chunks_indexed INTEGER NOT NULL DEFAULT 0, tokenizer TEXT NOT NULL DEFAULT 'porter unicode61', schema_version INTEGER NOT NULL DEFAULT 15, content_hash TEXT);`
    );
  ...
  it.skipIf(!sqliteVecAvailable)(
    'KNOWLEDGE_GRAPH type accepted in provenance after migration',
    () => {
      createV15Schema();
      migrateToLatest(db);
      ...
      const row = db
        ...
        .get('prov-kg-1') as { type: string; source_type: string };
  ...
  it.skipIf(!sqliteVecAvailable)(
    'KNOWLEDGE_GRAPH type NOT accepted before migration (v15 CHECK)',
    () => {
      createV15Schema();
  ...
  it.skipIf(!sqliteVecAvailable)(
    'can insert and query knowledge graph data after migration',
    () => {
      createV15Schema();
      migrateToLatest(db);
    ...
    `
      ...
      const node = db
        ...
        .get('kn-data') as Record<string, unknown>;
