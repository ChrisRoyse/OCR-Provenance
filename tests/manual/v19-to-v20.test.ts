...
describe('Migration v19 to v20 (Entity Embeddings, Temporal Edges, Node Scoring)', () => {
  ...
  function createV19Schema(): void {
    ...
    db.exec(
      `CREATE TABLE fts_index_metadata (id INTEGER PRIMARY KEY, last_rebuild_at TEXT, chunks_indexed INTEGER NOT NULL DEFAULT 0, tokenizer TEXT NOT NULL DEFAULT 'porter unicode61', schema_version INTEGER NOT NULL DEFAULT 19, content_hash TEXT);`
    );
  ...
  it.skipIf(!sqliteVecAvailable)('can insert temporal edge data after migration', () => {
    ...
    const edge = db.prepare('SELECT * FROM knowledge_edges WHERE id = ?').get('ke-v20') as Record<
      string,
      unknown
    >;
  ...
  it.skipIf(!sqliteVecAvailable)('existing data survives migration', () => {
    ...
    const doc = db.prepare('SELECT * FROM documents WHERE id = ?').get('doc-surv20') as Record<
      string,
      unknown
    >;
    ...
    const node = db
      ...
      .get('kn-surv20') as Record<string, unknown>;
