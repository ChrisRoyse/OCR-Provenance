...
describe('Migration v20 to v21 (Entity Embeddings Schema Fix)', () => {
  ...
  function createV20Schema(): void {
    ...
    db.exec(
      `CREATE TABLE fts_index_metadata (id INTEGER PRIMARY KEY, last_rebuild_at TEXT, chunks_indexed INTEGER NOT NULL DEFAULT 0, tokenizer TEXT NOT NULL DEFAULT 'porter unicode61', schema_version INTEGER NOT NULL DEFAULT 20, content_hash TEXT);`
    );
    ...
    db.exec(
      `CREATE VIRTUAL TABLE vec_entity_embeddings USING vec0(id TEXT PRIMARY KEY, embedding float[768] distance_metric=cosine);`
    );
    ...
    db.exec(
      'CREATE INDEX idx_knowledge_nodes_canonical_lower ON knowledge_nodes(canonical_name COLLATE NOCASE);'
    );
    ...
    db.exec(
      'CREATE INDEX idx_segments_doc_status ON entity_extraction_segments(document_id, extraction_status);'
    );
  ...
  it.skipIf(!sqliteVecAvailable)(
    'entity_embeddings rebuilt with correct schema after migration',
    () => {
      createV20Schema();
      migrateToLatest(db);
  ...
  it.skipIf(!sqliteVecAvailable)(
    'vec_entity_embeddings virtual table exists after migration',
    () => {
      createV20Schema();
      migrateToLatest(db);
  ...
  it.skipIf(!sqliteVecAvailable)(
    'can insert entity embedding with v21 schema after migration',
    () => {
      createV20Schema();
      migrateToLatest(db);
    ...
    `
      ...
      const emb = db.prepare('SELECT * FROM entity_embeddings WHERE id = ?').get('ee-1') as Record<
        string,
        unknown
      >;
  ...
  it.skipIf(!sqliteVecAvailable)('existing data survives migration', () => {
    ...
    const doc = db.prepare('SELECT * FROM documents WHERE id = ?').get('doc-surv21') as Record<
      string,
      unknown
    >;
    ...
    const node = db
      ...
      .get('kn-surv21') as Record<string, unknown>;
  ...
  it.skipIf(!sqliteVecAvailable)(
    'knowledge_nodes_fts has porter tokenizer after v22 migration',
    () => {
      createV20Schema();
      migrateToLatest(db);
