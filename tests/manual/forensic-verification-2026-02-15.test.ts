...
describe.skipIf(!sqliteVecAvailable)('F1: Cluster Reassignment INSERT', () => {
  ...
  it('reassignDocumentToCluster populates all NOT NULL columns in document_clusters', () => {
    ...
    const doc1 = insertSyntheticDocument(db, vector, 'doc1.pdf', 'Doc one text', [
      makeTestVector(0, 1),
    ]);
    const doc2 = insertSyntheticDocument(db, vector, 'doc2.pdf', 'Doc two text', [
      makeTestVector(0, 2),
    ]);

    // Insert entities for doc1 and create shared KG nodes
    const sharedNames = ['Alice Smith', 'Bob Jones'];
    const { nodeIds } = insertEntitiesAndKGNodes(
      conn,
      doc1.docId,
      doc1.chunkIds,
      sharedNames,
    ...
    const _clusterNow = new Date().toISOString();
    ...
    `
      ...
      .run(
        ...
        '{}',
        1,
        JSON.stringify(Array.from({ length: 768 }, () => 0)),
        0.9,
        0.8,
        computeHash('cluster-hash'),
        clusterProvId,
        100,
        now
      );
    ...
    const row = conn
      .prepare('SELECT * FROM document_clusters WHERE document_id = ? AND run_id = ?')
    ...
    console.error(
      '[F1 CHECK] similarity_to_centroid:',
      row!.similarity_to_centroid,
      '- PASS (not null, valid number)'
    );
    ...
    console.error(
      '[F1 CHECK] assigned_at:',
      row!.assigned_at,
      '- PASS (not null, valid ISO timestamp)'
    );

    // Verify membership_probability = 1.0
    expect(row!.membership_probability).toBe(1.0);
    console.error(
      '[F1 CHECK] membership_probability:',
...
describe.skipIf(!sqliteVecAvailable)('F2: Form-Fill Document Validation', () => {
  ...
  beforeEach(() => {
    ...
    db = DatabaseService.create(dbName, undefined, tempDir);
    const _vector = new VectorService(db.getConnection());
    state.currentDatabase = db;
    state.currentDatabaseName = dbName;
    conn = db.getConnection();
  });
  ...
  it('document_id is found via documents table but NOT via provenance with DOCUMENT type', () => {
    ...
    const correctQuery = conn.prepare('SELECT id FROM documents WHERE id = ?').get(docId) as
      | { id: string }
      | undefined;
    ...
    const brokenQuery = conn
      .prepare("SELECT id FROM provenance WHERE id = ? AND type = 'DOCUMENT'")
      .get(docId) as { id: string } | undefined;
    console.error(
      '[F2 CHECK] OLD broken query (provenance WHERE id = document_id) result:',
      brokenQuery
    );
    expect(brokenQuery).toBeUndefined();
    console.error(
      '[F2 CHECK] OLD broken query correctly does NOT find document (proving the bug was real) - PASS'
    );

    console.error(
      '[F2 VERDICT] PASS - Document validation now queries documents table, not provenance'
    );
  });
});
...
describe.skipIf(!sqliteVecAvailable)('F3-F6: Silent catch blocks now log errors', () => {
  ...
  it('F3: resolveEntityFilter with entity_names on empty entity_embeddings does not crash', () => {
    ...
    const entityRows = conn
      .prepare('SELECT document_id FROM entities WHERE normalized_text IN (?)')
    ...
    try {
      ...
      console.error(
        '[F3 CHECK] entity_embeddings table does not exist (expected for fresh DB without v20 migration)'
      );
    }
  ...
  it('F4: resolveEntityFilter with time_range on DB without temporal data does not crash', () => {
    ...
    console.error(
      '[F4 CHECK] Temporal edge query returned:',
      temporalRows.length,
      'rows (expected 0)'
    );
  ...
  it('F5: batchInferTemporalBounds returns empty map when no entity mentions exist', () => {
    ...
    console.error(
      '[F5 CHECK] Date entity query for non-existent chunk returned:',
      rows.length,
      'rows'
    );
    ...
    console.error(
      '[F5 VERDICT] PASS - Empty entity mentions handled gracefully (returns empty map)'
    );
  });
  ...
  it('F6: batchInferTemporalBoundsIncremental returns empty map with no matching data', () => {
    ...
    console.error(
      '[F6 CHECK] Incremental temporal query for non-existent chunk returned:',
      rows.length,
      'rows'
    );
    expect(rows.length).toBe(0);
...
describe.skipIf(!sqliteVecAvailable)(
  'F7: v17->v18 Migration FK on knowledge_nodes.provenance_id',
  () => {
    let tmpDir: string;
    ...
    `);
      ...
      rawDb.exec(
        `CREATE INDEX IF NOT EXISTS idx_kn_document_count ON knowledge_nodes(document_count)`
      );
      rawDb.exec(
    ...
    `
      ...
      console.error(
        '[F7 BEFORE] Inserted node with fake provenance_id (no FK) - succeeded as expected'
      );
      ...
      const schemaVersion = (
        rawDb.prepare('SELECT version FROM schema_version WHERE id = 1').get() as {
          version: number;
        }
      ).version;
      ...
      console.error(
        '[F7 VERDICT] PASS - FK on knowledge_nodes.provenance_id enforced after migration'
      );
    });
  }
);
...
describe.skipIf(!sqliteVecAvailable)('F9: Coreference page_number populated from chunks', () => {
  ...
  it('coreference-style entity_mention INSERT uses page_number from chunks table', () => {
    ...
    db.insertProvenance({
      ...
      root_document_id: docProvId,
      location: null,
      content_hash: computeHash(textContent),
      input_hash: null,
      file_hash: null,
      processor: 'datalab-marker',
      processor_version: '1.0.0',
    ...
    db.insertChunk({
      ...
      document_id: docId,
      ocr_result_id: ocrResultId,
      text: 'The patient was treated by Dr. Smith',
    ...
    const mention = conn
      .prepare('SELECT * FROM entity_mentions WHERE id = ?')
      .get(mentionId) as Record<string, unknown>;
    ...
    console.error(
      '[F9 CHECK] page_number:',
      mention.page_number,
      '- PASS (not null, equals chunk page_number 5)'
    );
...
describe('F11: FILE_MANAGER Error Categories Mapped Correctly', () => {
  it('FILE_MANAGER_API_ERROR maps to OCRAPIError', () => {
    ...
    console.error(
      '[F11 CHECK] FILE_MANAGER_API_ERROR ->',
  ...
  it('FILE_MANAGER_SERVER_ERROR maps to OCRAPIError', () => {
    ...
    console.error(
      '[F11 CHECK] FILE_MANAGER_SERVER_ERROR ->',
      error.constructor.name,
      'category:',
      error.category
    );
  ...
  it('FILE_MANAGER_FILE_ERROR maps to OCRFileError', () => {
    const error = mapPythonError('FILE_MANAGER_FILE_ERROR', 'File test error', {
      file_path: '/test/bad.pdf',
    });
    console.error(
      '[F11 CHECK] FILE_MANAGER_FILE_ERROR ->',
      error.constructor.name,
      'category:',
      error.category
    );
...
describe.skipIf(!sqliteVecAvailable)('F12: Comparison Handler Response Format', () => {
  ...
  it('ocr_comparison_list returns {success: true, data: {comparisons: [...]}}', async () => {
    ...
    console.error(
      '[F12] comparison_list format: {success: true, data: {comparisons: [...]}} - PASS'
    );
  });
...
describe.skipIf(!sqliteVecAvailable)('EC1: reassignDocumentToCluster with no entities', () => {
  ...
  it('returns {reassigned: false} without crashing when document has no entities', () => {
    ...
    const doc1 = insertSyntheticDocument(db, vector, 'no-entity.pdf', 'No entities here', [
      makeTestVector(0, 1),
    ]);
    const doc2 = insertSyntheticDocument(db, vector, 'other.pdf', 'Other document', [
      makeTestVector(0, 2),
    ]);
    ...
    console.error(
      '[EC1 VERDICT] PASS - Returns {reassigned: false, reason: "no KG nodes"} without crashing'
    );
  });
});
...
describe.skipIf(!sqliteVecAvailable)('EC2: Large entity_names list does not crash', () => {
  ...
  it('querying entities with 100+ names does not crash SQLite', () => {
    ...
    const entityNames = Array.from(
      { length: 150 },
      (_, i) => `Entity_${i}_${uuidv4().slice(0, 8)}`
    );
...
describe('EC3: mapPythonError with unknown category throws descriptive error', () => {
  it('unknown category throws OCRError with descriptive message', () => {
    ...
    console.error(
      '[EC3 VERDICT] PASS - Unknown category throws OCRError with category name and original message'
    );
  });
