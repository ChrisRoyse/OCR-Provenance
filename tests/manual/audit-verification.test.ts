...
import { initializeDatabase } from '../../src/services/storage/migrations/operations.js';
...
function _insertTestDocumentCluster(
  db: Database.Database,
  docId: string,
  clusterId: string,
  runId: string
...
function _insertTestEntityMention(
  db: Database.Database,
  entityId: string,
  documentId: string
): void {
...
describe('V1: F-PROV-1 -- Cluster reassignment provenance FK', () => {
  ...
  it.skipIf(!sqliteVecAvailable)(
    ...
    () => {
      ...
      `
        ...
        '1.0.0',
        '{}',
        JSON.stringify([provId]),
        2
      );

      // Verify FK constraint: source_id references provenance(id) - no error
      const provRow = db
        .prepare('SELECT source_id, root_document_id FROM provenance WHERE id = ?')
      ...
      expect(() => {
        ...
        `
          ...
          .run(
            ...
            'CLUSTERING',
            docId,
            docId, // docId is NOT a provenance ID
            'sha256:test',
            'cluster-reassign',
            '1.0.0',
            '{}',
  ...
  it('source code uses doc.provenance_id not input.document_id', () => {
    const clusteringSource = readFileSync(join(PROJECT_ROOT, 'src/tools/clustering.ts'), 'utf-8');

    // source_id comes BEFORE processor in the provenance object literal
    const sourceIdMatch = clusteringSource.match(/source_id:\s*(\S+?)[\s,]/);
...
describe('V2: F-SEC-1..F-SEC-4 -- Path sanitization', () => {
  ...
  it('sanitizePath with allowedBaseDirs rejects traversal outside', () => {
    ...
    expect(() => sanitizePath('/tmp/../etc/passwd', ['/tmp'])).toThrow(
...
describe('V3: F-TX-1 -- Incremental KG transactions', () => {
  it('incremental-builder.ts uses conn.transaction()', () => {
    const source = readFileSync(
      join(PROJECT_ROOT, 'src/services/knowledge-graph/incremental-builder.ts'),
      'utf-8'
    );
...
describe('V4: F-INTEG-4 -- form_fills cascade delete', () => {
  ...
  it.skipIf(!sqliteVecAvailable)(
    ...
    () => {
      ...
      const before = db
        .prepare('SELECT COUNT(*) as cnt FROM form_fills WHERE source_file_hash = ?')
      ...
      const docRow = db.prepare('SELECT file_hash FROM documents WHERE id = ?').get(docId) as
        | { file_hash: string }
        | undefined;
      if (docRow) {
  ...
  it('document-operations.ts contains form_fills deletion SQL', () => {
    const source = readFileSync(
      join(PROJECT_ROOT, 'src/services/storage/database/document-operations.ts'),
      'utf-8'
    );
...
describe('V5: F-INTEG-2 -- KG merge re-throws critical errors', () => {
  it('entity-analysis.ts re-throws FOREIGN KEY errors in catch block', () => {
    const source = readFileSync(join(PROJECT_ROOT, 'src/tools/entity-analysis.ts'), 'utf-8');
...
describe('V6: F-INTEG-3 -- Atomic document claiming', () => {
  ...
  it.skipIf(!sqliteVecAvailable)('atomic UPDATE transitions pending to processing', () => {
    const result = createFullTestDb(testDir);
    db = result.db;
    ...
    const processing = db
      .prepare("SELECT COUNT(*) as cnt FROM documents WHERE status = 'processing'")
      .get() as { cnt: number };
    expect(processing.cnt).toBe(1);

    // The other should still be pending
    const pending = db
      .prepare("SELECT COUNT(*) as cnt FROM documents WHERE status = 'pending'")
      .get() as { cnt: number };
...
describe('V7: F-SCHEMA-1 -- v24 migration index', () => {
  ...
  it('migrateV23ToV24 function exists in operations.ts', () => {
    const source = readFileSync(
      join(PROJECT_ROOT, 'src/services/storage/migrations/operations.ts'),
      'utf-8'
    );
...
describe('V8: F-PY-1 -- pythonPath', () => {
  it('clustering-service.ts has pythonPath: python3', () => {
    const source = readFileSync(
      join(PROJECT_ROOT, 'src/services/clustering/clustering-service.ts'),
      'utf-8'
    );
...
describe('V10: F-CONFIG-2 -- No console.warn', () => {
  it('pipeline.ts has zero console.warn calls', () => {
    const source = readFileSync(join(PROJECT_ROOT, 'src/services/vlm/pipeline.ts'), 'utf-8');
    const warns = source.match(/console\.warn\(/g);
    expect(warns).toBeNull();
  });

  it('optimizer.ts has zero console.warn calls', () => {
    const source = readFileSync(join(PROJECT_ROOT, 'src/services/images/optimizer.ts'), 'utf-8');
    const warns = source.match(/console\.warn\(/g);
    expect(warns).toBeNull();
  });
});
...
describe('V12: F-PROV-2 -- FORM_FILL depth 0', () => {
  ...
  it('FORM_FILL depth matches DOCUMENT depth (both are root-level)', () => {
    expect(PROVENANCE_CHAIN_DEPTH[ProvenanceType.FORM_FILL]).toBe(
      PROVENANCE_CHAIN_DEPTH[ProvenanceType.DOCUMENT]
    );
  });
});
...
describe('V13: F-INTEG-14 -- MAX(0,...) guard on document_count decrement', () => {
  ...
  it.skipIf(!sqliteVecAvailable)('MAX(0, document_count - 1) prevents negative counts', () => {
    const result = createFullTestDb(testDir);
    db = result.db;
    ...
    const row = db
      .prepare('SELECT document_count FROM knowledge_nodes WHERE id = ?')
  ...
  it('knowledge-graph-operations.ts uses MAX(0,...)', () => {
    const source = readFileSync(
      join(PROJECT_ROOT, 'src/services/storage/database/knowledge-graph-operations.ts'),
      'utf-8'
    );
...
describe('V14: F-INTEG-16 -- force param on IncrementalBuildOptions', () => {
  ...
  it('knowledge-graph.ts passes force param through', () => {
    const source = readFileSync(join(PROJECT_ROOT, 'src/tools/knowledge-graph.ts'), 'utf-8');
...
describe('V15: F-INTEG-6 -- Edge document_ids JSON pruning', () => {
  ...
  it.skipIf(!sqliteVecAvailable)(
    ...
    () => {
      ...
      const updatedEdge = db
        .prepare('SELECT document_ids, evidence_count FROM knowledge_edges WHERE id = ?')
...
describe('V16: F-INTEG-10 -- Stale comparison cleanup', () => {
  ...
  it.skipIf(!sqliteVecAvailable)('existing comparison is deleted before new insert', () => {
    const result = createFullTestDb(testDir);
    db = result.db;
    ...
    const rows = db
      .prepare('SELECT id FROM comparisons WHERE document_id_1 = ?')
  ...
  it.skipIf(!sqliteVecAvailable)(
    ...
    () => {
      ...
      const linksBefore = db
        .prepare('SELECT COUNT(*) as cnt FROM node_entity_links WHERE entity_id = ?')
...
describe('V18: F-INTEG-13 -- busy_timeout 30000', () => {
  it('schema-definitions.ts has busy_timeout = 30000', () => {
    const source = readFileSync(
      join(PROJECT_ROOT, 'src/services/storage/migrations/schema-definitions.ts'),
      'utf-8'
    );
  ...
  it.skipIf(!sqliteVecAvailable)('initialized database has busy_timeout >= 30000', () => {
...
describe('V19: F-INTEG-15 -- Orphan node cleanup in entity deletion', () => {
  it('entity-operations.ts contains orphan node cleanup logic', () => {
    const source = readFileSync(
      join(PROJECT_ROOT, 'src/services/storage/database/entity-operations.ts'),
      'utf-8'
    );
  ...
  it.skipIf(!sqliteVecAvailable)('orphan nodes are cleaned up after entity deletion', () => {
    const result = createFullTestDb(testDir);
    db = result.db;
    ...
    const nodeGone = db
      .prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes WHERE id = ?')
      .get(nodeId) as { cnt: number };
    expect(nodeGone.cnt).toBe(0);

    const edgeGone = db
      .prepare('SELECT COUNT(*) as cnt FROM knowledge_edges WHERE id = ?')
      .get(edgeId) as { cnt: number };
    expect(edgeGone.cnt).toBe(0);

    // Non-orphan node still exists
    const otherNodeExists = db
      .prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes WHERE id = ?')
...
describe('V21: F-INTEG-5 -- Stale embedding detection (removed - dead code)', () => {
  it('detectStaleEmbeddings was removed as dead code', () => {
    const source = readFileSync(join(PROJECT_ROOT, 'src/services/embedding/embedder.ts'), 'utf-8');
    expect(source).not.toContain('export function detectStaleEmbeddings');
  });
});
...
describe('V25: F-INTEG-12 -- SIGKILL timer race fix', () => {
  it('extractor.ts settles the promise after SIGKILL', () => {
    const source = readFileSync(join(PROJECT_ROOT, 'src/services/images/extractor.ts'), 'utf-8');
...
describe('V26: F-SCHEMA-3 -- model provenance depth consistency', () => {
  it('provenance model comment matches value for FORM_FILL', () => {
    const source = readFileSync(join(PROJECT_ROOT, 'src/models/provenance.ts'), 'utf-8');
