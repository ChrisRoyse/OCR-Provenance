...
function insertSyntheticDocument(
  ...
  for (let ci = 0; ci < chunkCount; ci++) {
    ...
    db.insertChunk({
      ...
      chunk_index: ci,
      character_start: ci * 200,
      character_end: (ci + 1) * 200,
      page_number: 1,
      page_range: null,
      overlap_previous: 0,
      overlap_next: 0,
  ...
  db.insertProvenance({
    ...
    input_hash: null,
    file_hash: null,
    processor: 'gemini-entity-extractor',
    processor_version: '1.0.0',
    processing_params: {},
...
const DOC_A_ENTITIES = [
  ...
  { entityType: 'person', rawText: 'Jane Doe', normalizedText: 'jane doe', confidence: 0.9 },
  {
    entityType: 'organization',
    rawText: 'Smith & Associates Corp.',
    normalizedText: 'smith & associates corp.',
    confidence: 0.88,
  },
  {
    entityType: 'location',
    rawText: 'New York City',
    normalizedText: 'new york city',
    confidence: 0.92,
  },
  {
    entityType: 'case_number',
    rawText: '2024-CV-12345',
    normalizedText: '2024-cv-12345',
    confidence: 0.95,
  },
];
...
const DOC_B_ENTITIES = [
  {
    entityType: 'person',
    rawText: 'John D. Smith',
    normalizedText: 'john d. smith',
    confidence: 0.91,
  },
  {
    entityType: 'person',
    rawText: 'Dr. Robert Johnson',
    normalizedText: 'dr. robert johnson',
    confidence: 0.87,
  },
  {
    entityType: 'organization',
    rawText: 'Smith & Associates Corporation',
    normalizedText: 'smith & associates corporation',
    confidence: 0.85,
  },
  { entityType: 'location', rawText: 'New York', normalizedText: 'new york', confidence: 0.9 },
  { entityType: 'date', rawText: '2024-03-15', normalizedText: '2024-03-15', confidence: 0.95 },
];

const DOC_C_ENTITIES = [
  { entityType: 'person', rawText: 'J. Smith', normalizedText: 'j. smith', confidence: 0.8 },
...
describe('Knowledge Graph Manual Verification', () => {
  ...
  describe('Pre-conditions: Verify synthetic data setup', () => {
    ...
    it('should have 15 entities across 3 documents', () => {
      ...
      const docACnt = (
        conn
          .prepare('SELECT COUNT(*) as cnt FROM entities WHERE document_id = ?')
          .get(docA.docId) as any
      ).cnt;
      const docBCnt = (
        conn
          .prepare('SELECT COUNT(*) as cnt FROM entities WHERE document_id = ?')
          .get(docB.docId) as any
      ).cnt;
      const docCCnt = (
        conn
          .prepare('SELECT COUNT(*) as cnt FROM entities WHERE document_id = ?')
          .get(docC.docId) as any
      ).cnt;
    ...
    it('should have 15 entity_mentions linked to chunks', () => {
      ...
      const withChunks = (
        conn
          .prepare('SELECT COUNT(*) as cnt FROM entity_mentions WHERE chunk_id IS NOT NULL')
          .get() as any
      ).cnt;
    ...
    it('should have correct provenance chain for each document', () => {
      ...
      for (const [label, doc] of [
        ...
        const docProv = conn
          ...
          .get(doc.docProvId) as any;
        const ocrProv = conn
          .prepare('SELECT * FROM provenance WHERE id = ?')
          .get(doc.ocrProvId) as any;
        const entityProv = conn
          .prepare('SELECT * FROM provenance WHERE id = ?')
          .get(doc.entityExtProvId) as any;
        ...
        console.error(
          `  ENTITY_EXTRACTION: depth=${entityProv.chain_depth}, type=${entityProv.type}`
        );
    ...
    it('knowledge graph tables should be empty before build', () => {
      ...
      const links = (conn.prepare('SELECT COUNT(*) as cnt FROM node_entity_links').get() as any)
        .cnt;
  ...
  describe('Graph Build Verification', () => {
    it('should build knowledge graph with fuzzy resolution and verify all DB records', async () => {
      ...
      const nodeCount = (conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes').get() as any)
        .cnt;
      console.error(`=== SOURCE OF TRUTH: knowledge_nodes ===`);
      console.error(
        `  count: ${nodeCount} (matches result.total_nodes: ${nodeCount === result.total_nodes})`
      );
      expect(nodeCount).toBe(result.total_nodes);

      // PHYSICAL VERIFICATION: All 15 entities should be linked
      const linkCount = (conn.prepare('SELECT COUNT(*) as cnt FROM node_entity_links').get() as any)
        .cnt;
      ...
      const edgeCount = (conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_edges').get() as any)
        .cnt;
      console.error(`=== SOURCE OF TRUTH: knowledge_edges ===`);
      console.error(
        `  count: ${edgeCount} (matches result.total_edges: ${edgeCount === result.total_edges})`
      );
      ...
      const provCount = (
        conn
          .prepare("SELECT COUNT(*) as cnt FROM provenance WHERE type = 'KNOWLEDGE_GRAPH'")
          .get() as any
      ).cnt;
  ...
  describe('Entity Resolution Correctness', () => {
    ...
    it('should merge "John Smith", "John D. Smith", and "J. Smith" into ONE node', () => {
      ...
      const allPersonNodes = conn
        .prepare("SELECT * FROM knowledge_nodes WHERE entity_type = 'person'")
        .all() as any[];

      console.error('=== ENTITY RESOLUTION: John Smith variants ===');
      for (const node of allPersonNodes) {
        console.error(
          `  node: "${node.canonical_name}" (normalized: "${node.normalized_name}"), doc_count=${node.document_count}, mention_count=${node.mention_count}`
        );
      }

      const smithNodes = allPersonNodes.filter(
        (n) =>
      ...
      for (const link of links) {
        console.error(
          `  entity: "${link.raw_text}", doc: ${link.document_id}, similarity: ${link.similarity_score}`
        );
      }
    ...
    it('should merge "Smith & Associates Corp." and "Smith & Associates Corporation" into ONE node', () => {
      ...
      for (const node of orgNodes) {
        console.error(
          `  node: "${node.canonical_name}" (normalized: "${node.normalized_name}"), doc_count=${node.document_count}, aliases=${node.aliases}`
        );
      }
    ...
    it('should merge "New York City" and "New York" into ONE node (location containment)', () => {
      ...
      for (const node of nyNodes) {
        console.error(
          `  node: "${node.canonical_name}" (normalized: "${node.normalized_name}"), doc_count=${node.document_count}`
        );
      }
    ...
    it('should keep "Manhattan" and "NYPD" and "Dr. Robert Johnson" as separate single-document nodes', () => {
      ...
      const manhattan = conn
        .prepare("SELECT * FROM knowledge_nodes WHERE normalized_name LIKE '%manhattan%'")
        .all() as any[];
      console.error('=== SINGLE-DOC NODES ===');
      console.error(
        `  Manhattan: ${manhattan.length} node(s), doc_count=${manhattan[0]?.document_count}`
      );
      expect(manhattan).toHaveLength(1);
      expect(manhattan[0].document_count).toBe(1);

      const nypd = conn
        .prepare("SELECT * FROM knowledge_nodes WHERE normalized_name LIKE '%nypd%'")
        .all() as any[];
      ...
      const robert = conn
        .prepare("SELECT * FROM knowledge_nodes WHERE normalized_name LIKE '%robert%'")
        .all() as any[];
      console.error(
        `  Dr. Robert Johnson: ${robert.length} node(s), doc_count=${robert[0]?.document_count}`
      );
      expect(robert).toHaveLength(1);
      expect(robert[0].document_count).toBe(1);

      const date = conn
        .prepare("SELECT * FROM knowledge_nodes WHERE entity_type = 'date'")
        .all() as any[];
    ...
    it('should have correct total node count after resolution', () => {
      ...
      const totalNodes = (conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes').get() as any)
        .cnt;
  ...
  describe('Co-occurrence Edge Verification', () => {
    ...
    it('should have co_mentioned edges between entities sharing documents', () => {
      ...
      const coMentionedEdges = conn
        .prepare("SELECT * FROM knowledge_edges WHERE relationship_type = 'co_mentioned'")
        .all() as any[];
      ...
      for (const edge of coMentionedEdges) {
        ...
        console.error(
          `  edge: ${edge.source_node_id.slice(0, 8)}..${edge.target_node_id.slice(0, 8)} weight=${edge.weight} docs=${docIds.length}`
        );
      }
    });
    ...
    it('should have edges with correct weight calculation (0 < weight <= 1)', () => {
      ...
      for (const edge of edges) {
        console.error(
          `  ${edge.relationship_type}: weight=${edge.weight}, evidence=${edge.evidence_count}`
        );
    ...
    it('should have co_located edges for entities sharing chunks', () => {
      ...
      const coLocatedEdges = conn
        .prepare("SELECT * FROM knowledge_edges WHERE relationship_type = 'co_located'")
        .all() as any[];
      ...
      if (coLocatedEdges.length > 0) {
        for (const edge of coLocatedEdges) {
          ...
          console.error(
            `  edge weight=${edge.weight}, shared_chunks=${meta.shared_chunk_ids?.length || 0}`
          );
          expect(edge.weight).toBeGreaterThan(0);
  ...
  describe('Provenance Verification', () => {
    ...
    it('should have KNOWLEDGE_GRAPH provenance record(s)', () => {
      ...
      const provRecords = conn
        .prepare("SELECT * FROM provenance WHERE type = 'KNOWLEDGE_GRAPH'")
        .all() as any[];

      console.error('=== KNOWLEDGE_GRAPH PROVENANCE ===');
      for (const prov of provRecords) {
        console.error(
          `  id=${prov.id}, processor=${prov.processor}, chain_depth=${prov.chain_depth}, source_type=${prov.source_type}`
        );
      }
      ...
      const mainRecords = provRecords.filter(
        (p: Record<string, unknown>) => p.processor === 'knowledge-graph-builder'
      );
  ...
  describe('Path Finding Verification', () => {
    ...
    it('finds paths between entities sharing documents', () => {
      ...
      for (const path of result.paths) {
        console.error(
          `  path length=${path.length}, nodes=${path.nodes.map((n) => n.canonical_name).join(' -> ')}`
        );
    ...
    it('throws error for nonexistent entity', () => {
      expect(() => findGraphPaths(db, 'Nonexistent Person', 'Jane Doe')).toThrow(
  ...
  describe('Graph Stats Verification', () => {
    ...
    it('stats match direct database queries', () => {
      ...
      // Direct counts
      const directNodeCount = (
        conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes').get() as any
      ).cnt;
      const directEdgeCount = (
        conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_edges').get() as any
      ).cnt;
      const directLinkCount = (
        conn.prepare('SELECT COUNT(*) as cnt FROM node_entity_links').get() as any
      ).cnt;
      const directCrossDoc = (
        conn
          .prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes WHERE document_count > 1')
          .get() as any
      ).cnt;
      const directDocsCovered = (
        conn
      ...
      console.error(
        `  docs_covered: stats=${stats.documents_covered}, direct=${directDocsCovered}`
      );
  ...
  describe('Cascade Delete Verification', () => {
    ...
    it('deleting a document updates graph correctly', () => {
      ...
      console.error(
        `  nodes: ${nodeCountBefore}, links: ${linkCountBefore}, edges: ${edgeCountBefore}`
      );
      ...
      console.error(
        `  links_deleted: ${result.links_deleted}, nodes_deleted: ${result.nodes_deleted}, edges_deleted: ${result.edges_deleted}`
      );
      ...
      const smithNode = conn
        .prepare(
          "SELECT * FROM knowledge_nodes WHERE entity_type = 'person' AND normalized_name LIKE '%john%smith%'"
        )
        .all() as any[];
      console.error(
        `  John Smith: ${smithNode.length} node(s), doc_count=${smithNode[0]?.document_count}`
      );
      ...
      const janeDoe = conn
        .prepare("SELECT * FROM knowledge_nodes WHERE normalized_name LIKE '%jane doe%'")
        .all() as any[];
      console.error(
        `  Jane Doe: ${janeDoe.length} node(s), doc_count=${janeDoe[0]?.document_count}`
      );
      ...
      const caseNode = conn
        .prepare("SELECT * FROM knowledge_nodes WHERE entity_type = 'case_number'")
        .all() as any[];
      console.error(
        `  Case 2024-CV-12345: ${caseNode.length} node(s), doc_count=${caseNode[0]?.document_count}`
      );
  ...
  describe('Rebuild Idempotency', () => {
    it('rebuild produces consistent graph after full delete', async () => {
      ...
      // Verify clean state
      const nodeCountAfterDelete = (
        conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes').get() as any
      ).cnt;
      const edgeCountAfterDelete = (
        conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_edges').get() as any
      ).cnt;
  ...
  describe('Tool Handlers: build, query, node, paths, stats, delete', () => {
    ...
    it('ocr_knowledge_graph_node returns detailed node info', async () => {
      ...
      const smithNode = conn
        .prepare("SELECT id FROM knowledge_nodes WHERE canonical_name = 'John Smith'")
        .get() as { id: string };
    ...
    it('ocr_knowledge_graph_delete removes all data and verifies in DB', async () => {
      ...
      // Verify graph exists
      const beforeNodes = (conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes').get() as any)
        .cnt;
      ...
      const afterNodes = (conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_nodes').get() as any)
        .cnt;
      const afterEdges = (conn.prepare('SELECT COUNT(*) as cnt FROM knowledge_edges').get() as any)
        .cnt;
      const afterLinks = (
        conn.prepare('SELECT COUNT(*) as cnt FROM node_entity_links').get() as any
      ).cnt;
      console.error(
        `  after delete: nodes=${afterNodes}, edges=${afterEdges}, links=${afterLinks}`
      );
  ...
  describe('Edge Cases', () => {
    it('building graph with no entities fails gracefully', async () => {
      ...
      // Build should fail
      await expect(buildKnowledgeGraph(db, { resolution_mode: 'fuzzy' })).rejects.toThrow(
        'No documents provided for KG build'
      );
    ...
    it('exact resolution mode does not merge fuzzy variants', async () => {
      ...
      const janeDoeNodes = conn
        .prepare("SELECT * FROM knowledge_nodes WHERE normalized_name = 'jane doe'")
        .all() as any[];
    ...
    it('graph with single document still works', async () => {
      ...
      console.error(
        `  nodes: ${result.total_nodes}, edges: ${result.total_edges}, docs: ${result.documents_covered}`
      );
