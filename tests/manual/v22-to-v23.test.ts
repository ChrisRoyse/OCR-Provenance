...
describe('Migration v22 to v23 (Medical Relationship Types)', () => {
  ...
  function insertTestData(): void {
    ...
    db.prepare(
      `INSERT INTO provenance VALUES ('prov-kg-1', 'KNOWLEDGE_GRAPH', ?, ?, 'KNOWLEDGE_GRAPH', 'doc1', 'hash1', 'test', '1.0', '{}', '[]', 2)`
    ).run(now, now);
    db.prepare(
      `INSERT INTO knowledge_nodes VALUES ('n1', 'person', 'Dr. Smith', 'dr. smith', NULL, 1, 5, 2, 0.9, NULL, 'prov-kg-1', ?, ?, NULL, NULL)`
    ).run(now, now);
    db.prepare(
      `INSERT INTO knowledge_nodes VALUES ('n2', 'medication', 'Aspirin', 'aspirin', NULL, 1, 3, 1, 0.85, NULL, 'prov-kg-1', ?, ?, NULL, NULL)`
    ).run(now, now);
    db.prepare(
      `INSERT INTO knowledge_nodes VALUES ('n3', 'organization', 'City Hospital', 'city hospital', NULL, 2, 10, 3, 0.88, NULL, 'prov-kg-1', ?, ?, NULL, NULL)`
    ).run(now, now);

    // Insert edges with existing relationship types
    db.prepare(
      `INSERT INTO knowledge_edges VALUES ('e1', 'n1', 'n3', 'works_at', 1.5, 3, '["doc1"]', NULL, 'prov-kg-1', ?, NULL, NULL, 0.5, 0)`
    ).run(now);
    db.prepare(
      `INSERT INTO knowledge_edges VALUES ('e2', 'n1', 'n2', 'co_mentioned', 1.0, 1, '["doc1"]', NULL, 'prov-kg-1', ?, NULL, NULL, 0.3, 0)`
    ).run(now);
    db.prepare(
      `INSERT INTO knowledge_edges VALUES ('e3', 'n2', 'n3', 'related_to', 0.8, 2, '["doc1"]', '{"context":"prescription"}', 'prov-kg-1', ?, '2026-01-01', '2026-12-31', 0.2, 1)`
    ).run(now);
  }
  ...
  it.skipIf(!sqliteVecAvailable)(
    'CHECK constraint includes medical relationship types after migration',
    () => {
      createV22Schema();
      migrateToLatest(db);
  ...
  it.skipIf(!sqliteVecAvailable)(
    'CHECK constraint still includes all 12 original relationship types',
    () => {
      createV22Schema();
      migrateToLatest(db);
  ...
  it.skipIf(!sqliteVecAvailable)(
    'new medical relationship types can be inserted after migration',
    () => {
      createV22Schema();
      insertTestData();
      migrateToLatest(db);
      ...
      const count = (
        db.prepare('SELECT COUNT(*) as cnt FROM knowledge_edges').get() as { cnt: number }
      ).cnt;
  ...
  it.skipIf(!sqliteVecAvailable)(
    'migration skips gracefully when knowledge_edges table does not exist',
    () => {
