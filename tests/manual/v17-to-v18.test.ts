...
describe('Migration v17 to v18 (Medical Entity Types)', () => {
  ...
  function createV17Schema(): void {
    ...
    db.exec(
      `CREATE TABLE fts_index_metadata (id INTEGER PRIMARY KEY, last_rebuild_at TEXT, chunks_indexed INTEGER NOT NULL DEFAULT 0, tokenizer TEXT NOT NULL DEFAULT 'porter unicode61', schema_version INTEGER NOT NULL DEFAULT 17, content_hash TEXT);`
    );
  ...
  it.skipIf(!sqliteVecAvailable)('entities table accepts medication type after migration', () => {
    ...
    const row = db.prepare('SELECT entity_type FROM entities WHERE id = ?').get('ent-med-1') as {
      entity_type: string;
    };
  ...
  it.skipIf(!sqliteVecAvailable)(
    'entities table accepts medical_device type after migration',
    () => {
      createV17Schema();
      migrateToLatest(db);
  ...
  it.skipIf(!sqliteVecAvailable)(
    'medication type NOT accepted before migration (v17 CHECK)',
    () => {
      createV17Schema();
  ...
  it.skipIf(!sqliteVecAvailable)(
    'knowledge_nodes table accepts medication type after migration',
    () => {
      createV17Schema();
      migrateToLatest(db);
      ...
      const row = db
        ...
        .get('kn-med') as { entity_type: string };
      expect(row.entity_type).toBe('medication');
    }
  );

  it.skipIf(!sqliteVecAvailable)(
    'knowledge_nodes table accepts diagnosis type after migration',
    () => {
      createV17Schema();
      migrateToLatest(db);
  ...
  it.skipIf(!sqliteVecAvailable)(
    'knowledge_nodes has importance_score and resolution_type after migration',
    () => {
      createV17Schema();
      migrateToLatest(db);
  ...
  it.skipIf(!sqliteVecAvailable)('preserves existing entities during migration', () => {
    ...
    const entity = db.prepare('SELECT * FROM entities WHERE id = ?').get('ent-surv-1') as Record<
      string,
      unknown
    >;
  ...
  it.skipIf(!sqliteVecAvailable)('preserves existing knowledge_nodes during migration', () => {
    ...
    const node = db.prepare('SELECT * FROM knowledge_nodes WHERE id = ?').get('kn-surv') as Record<
      string,
      unknown
    >;
