...
function setupDatabase(): Database.Database {
  ...
  const { DatabaseService } = require(resolve(
    PROJECT_ROOT,
    'dist/services/storage/database/index.js'
  ));
...
function insertSyntheticData(db: Database.Database): {
  ...
  for (let i = 0; i < chunkTexts.length; i++) {
    ...
    `).run(
      chunkProvId, now, now, targetProvDocId, targetDocId,
      sha256(chunkTexts[i]), sha256('ocr text'), sha256('file1'),
      targetProvDocId
    );
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?, 'pending', 'hybrid_section',
        ?, ?, '["text"]', 0)
    `).run(
      chunkId, targetDocId, targetOcrId, chunkTexts[i], sha256(chunkTexts[i]),
      i, i * 100, (i + 1) * 100, i + 1, chunkProvId, now,
...
function testH1H2_FTS5QuerySanitization(db: Database.Database) {
  ...
  const { expandQuery, sanitizeFTS5Term } = require(resolve(
    PROJECT_ROOT,
    'dist/services/search/query-expander.js'
  ));
  ...
  const bm25Service = new (require(resolve(PROJECT_ROOT, 'dist/services/search/bm25.js')).BM25SearchService)(db);
  ...
  const resultTexts = (searchResult as Array<{ original_text: string }>)
    .map(r => r.original_text.toLowerCase());
...
function testH3H4_MigrationSafety() {
  ...
  assert(
    txnPattern.test(opsContent),
    'v31 migration: bumpVersion(31) is inside db.transaction()',
  );
...
function testH5_CrossDBNormalization() {
  ...
  assert(
    searchContent.includes(': 1.0'),
    'Single-result databases get normalized_score = 1.0',
  );
}
...
function testH6_UnboundedQueries(db: Database.Database) {
  ...
  assert(
    getAllTagsSql.test(tagOpsContent),
    'getAllTags() SQL includes LIMIT clause',
  );
  ...
  const { getAllTags } = require(resolve(PROJECT_ROOT, 'dist/services/storage/database/tag-operations.js'));
  const tags = getAllTags(db);
  assert(
    Array.isArray(tags),
    'getAllTags() returns an array (with LIMIT clause)',
  );

  // Also verify provenance operations have limits
  const provOpsPath = resolve(PROJECT_ROOT, 'dist/services/storage/database/provenance-operations.js');
...
function testM1_MetadataFilterBypass(db: Database.Database, _docId: string) {
  ...
  const row = db.prepare('SELECT COUNT(*) as cnt FROM documents WHERE id = ?').get(sentinel) as { cnt: number };
  assert(
    row.cnt === 0,
    'Sentinel "__no_match__" never matches a real document',
  );
}
...
function testM3_LIKEEscaping(db: Database.Database, _docId2: string) {
  ...
  const literalResult = db.prepare(
    "SELECT COUNT(*) as cnt FROM chunks WHERE section_path LIKE ? ESCAPE '\\'"
...
function testL8_ErrorIsErrorFlag() {
  ...
  assert(
    ...
    'handleError content[0].type is "text"',
  );
...
function testL13_ProvenanceCircularRef(db: Database.Database) {
  ...
  const { getProvenanceChain } = require(resolve(
    PROJECT_ROOT,
...
function testL3_ParseIntNaN() {
  ...
  assert(
    errMsg.includes('Invalid numeric') || errMsg.includes('NaN') || errMsg.includes('GEMINI_MAX_OUTPUT_TOKENS'),
...
function testL1_EmbeddingDeviceResolution() {
  ...
  try {
    ...
    assert(
      result !== 'auto',
      `Python resolve_device("auto") returns concrete device: "${result}"`,
    );
  } catch (err) {
...
function testAdditionalSourceVerifications() {
  ...
  const provOpsContent = readFileSync(provOpsPath, 'utf-8');
  assert(
    provOpsContent.includes('VALID_ORDER_COLUMNS') || provOpsContent.includes('Invalid order column'),
  ...
  assert(
    qualityContent.includes('Math.min') || qualityContent.includes('Math.max') || qualityContent.includes('clamp'),
  ...
  if (existsSync(chunkerPath)) {
    ...
    assert(
      chunkerContent.includes('trim()') || chunkerContent.includes('length === 0') ||
