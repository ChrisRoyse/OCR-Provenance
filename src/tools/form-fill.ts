...
import { listKnowledgeNodes } from '../services/storage/database/knowledge-graph-operations.js';
import {
  insertEntity,
  insertEntityMention,
...
const FormFillInput = z.object({
  ...
  document_id: z
    ...
    .optional()
    .describe(
      'If provided, validate field values against entities from this document and extract new entities from filled values'
    ),
  validate_against_kg: z
    .boolean()
    .default(false)
    .describe(
      'If true and document_id provided, validate filled values against knowledge graph nodes and relationships'
    ),
});
...
function validateFieldsAgainstEntities(
  ...
  for (const [fieldName, fieldInfo] of Object.entries(fieldData)) {
    ...
    for (const entity of entities) {
      ...
      if (
        entity.normalized_text === normalizedValue ||
        entity.raw_text.toLowerCase().trim() === normalizedValue
      ) {
...
function validateFieldsAgainstKG(
  ...
  field: string;
  value: string;
  kg_node_match: string | null;
  kg_node_type: string | null;
  relationship_verified: boolean | null;
}> {
  const kgNodes = conn
    .prepare(
      `
  ...
  for (const [fieldName, fieldInfo] of Object.entries(fieldData)) {
    ...
    if (matchedNode && inferredType) {
      const isRelationshipField = /employer|employee|doctor|patient|attorney|client/.test(
...
function extractEntitiesFromFilledFields(
  ...
  const existingEntities = conn
    .prepare('SELECT normalized_text, entity_type FROM entities WHERE document_id = ?')
    .all(documentId) as Array<{ normalized_text: string; entity_type: string }>;
  const existingSet = new Set(
    existingEntities.map((e) => `${e.entity_type}::${e.normalized_text}`)
  );
...
async function handleFormFill(params: Record<string, unknown>) {
  try {
    ...
    if (input.document_id) {
      ...
      if (input.validate_against_kg) {
        try {
          ...
          console.error(
            `[WARN] KG validation failed: ${e instanceof Error ? e.message : String(e)}`
          );
        }
      }
    }
    ...
    let entityExtraction:
      | { entities_created: number; entity_ids: string[]; entity_provenance_id: string }
      | undefined;
    if (input.document_id) {
      try {
        // Look up document details for provenance chain via documents table
        const docRecord = conn
          .prepare(
        ...
        console.error(
          `[WARN] Post-fill entity extraction failed: ${e instanceof Error ? e.message : String(e)}`
        );
      }
    }
...
const FormFillSuggestFieldsInput = z.object({
  ...
  field_names: z
    ...
    .min(1)
    .describe(
      'Field names to find entity values for (e.g., ["patient_name", "date_of_birth", "address"])'
    ),
  entity_types: z
    .array(
      z.enum([
    ...
    .optional()
    .describe('Limit suggestions to specific entity types'),
});
...
function inferEntityTypeFromFieldName(fieldName: string): string | null {
  ...
  if (/org|company|employer|hospital|clinic|facility|institution|firm/.test(lower))
    return 'organization';
...
async function handleFormFillSuggestFields(params: Record<string, unknown>) {
  try {
    ...
    try {
      ...
      console.error(
        `[form-fill] KG nodes query in suggest_fields failed: ${err instanceof Error ? err.message : String(err)}`
      );
...
export const formFillTools: Record<string, ToolDefinition> = {
  ocr_form_fill: {
    description:
      'Fill a PDF or image form using Datalab API. Provide field names and values, optionally save filled form to disk. When document_id is provided, validates field values against extracted entities and creates entity records for new values. Set validate_against_kg=true for KG node/relationship validation.',
  ...
  ocr_form_fill_suggest_fields: {
    description:
      'Suggest field values for form filling from extracted entities and knowledge graph. Maps field names to entity types and returns ranked suggestions.',
