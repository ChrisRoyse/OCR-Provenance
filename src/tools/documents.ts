...
export async function handleDocumentList(params: Record<string, unknown>): Promise<ToolResponse> {
  try {
    ...
    const total = input.status_filter
      ? (stats.documents_by_status[input.status_filter as keyof typeof stats.documents_by_status] ??
        0)
    ...
    if (input.include_entity_counts) {
      ...
      try {
        ...
        console.error(
          `[documents] entity/KG count query failed: ${err instanceof Error ? err.message : String(err)}`
        );
    ...
    const hasEntityFilters =
      input.entity_type_filter || input.min_entity_count || input.entity_name_filter;
    if (hasEntityFilters) {
      const conn = db.getConnection();
      const docIds = documents.map((d) => d.id);
      if (docIds.length > 0) {
        ...
        try {
          ...
          if (input.entity_type_filter && input.entity_type_filter.length > 0) {
            ...
            allowedDocIds = allowedDocIds
              ? new Set([...allowedDocIds].filter((id) => typeSet.has(id)))
              : typeSet;
          }
          ...
          if (input.min_entity_count) {
            const rows = conn
              ...
              .all(...docIds, input.min_entity_count) as Array<{
              document_id: string;
              cnt: number;
            }>;
            const countSet = new Set(rows.map((r) => r.document_id));
            allowedDocIds = allowedDocIds
              ? new Set([...allowedDocIds].filter((id) => countSet.has(id)))
              : countSet;
          }
          ...
          if (input.entity_name_filter) {
            ...
            allowedDocIds = allowedDocIds
              ? new Set([...allowedDocIds].filter((id) => nameSet.has(id)))
              : nameSet;
          }
        } catch (err) {
          console.error(
            `[documents] entity filter query failed: ${err instanceof Error ? err.message : String(err)}`
          );
  ...
  try {
    ...
    if (input.include_entity_summary) {
      ...
      try {
        ...
        let extractionCoverage: { total_segments: number; segments_with_entities: number } | null =
          null;
        try {
          const coverage = conn
            .prepare(
          ...
          console.error(
            `[documents] entity_extraction_segments query failed: ${err instanceof Error ? err.message : String(err)}`
          );
        ...
        console.error(
          `[documents] entity summary query failed: ${err instanceof Error ? err.message : String(err)}`
        );
...
export const documentTools: Record<string, ToolDefinition> = {
  ocr_document_list: {
    description:
      'List documents in the current database. Supports entity-based filtering: entity_type_filter, min_entity_count, entity_name_filter. Set include_entity_counts=true to see entity and KG node counts per document.',
    inputSchema: {
      status_filter: z
  ...
  ocr_document_get: {
    description:
      'Get detailed information about a specific document. Set include_entity_summary=true for per-type entity counts, top entities, and extraction coverage.',
    inputSchema: {
