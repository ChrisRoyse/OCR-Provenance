 ...
 */
export function insertEntity(db: Database.Database, entity: Entity): string {
 ...
 */
export function insertEntityMention(db: Database.Database, mention: EntityMention): string {
  const stmt = db.prepare(`
...
export function getEntitiesByDocument(db: Database.Database, documentId: string): Entity[] {
  return db
    .prepare('SELECT * FROM entities WHERE document_id = ? ORDER BY entity_type, normalized_text')
    ...
    if (affectedNodeIds.length > 0) {
      ...
      if (orphanNodes.length > 0) {
        ...
        try {
          ...
          db.prepare(`DELETE FROM entity_embeddings WHERE node_id IN (${orphanPlaceholders})`).run(
            ...orphanIds
          );
        } catch (e: unknown) {
        ...
        db.prepare(`DELETE FROM knowledge_nodes WHERE id IN (${orphanPlaceholders})`).run(
          ...orphanIds
        );
      }
    }
 ...
 */
export function getEntitiesByDocumentKeyed(
  db: Database.Database,
  documentId: string
): Map<string, Entity> {
...
export function getEntityTypeDistribution(
  ...
  try {
    ...
    console.error(
      `[entity-operations] getEntityTypeDistribution failed for document ${documentId}:`,
 ...
 */
export function getEntityCount(db: Database.Database, documentId: string): number {
  try {
    return (
      db.prepare('SELECT COUNT(*) as cnt FROM entities WHERE document_id = ?').get(documentId) as {
        cnt: number;
      }
    ).cnt;
  } catch (e) {
    console.error(
      `[entity-operations] getEntityCount failed for document ${documentId}:`,
      e instanceof Error ? e.message : String(e)
    );
    return 0;
  }
 ...
 * Returns 0 if entity tables do not exist.
 */
export function getPagesWithEntities(db: Database.Database, documentId: string): number {
  try {
    return (
      db
        .prepare(
    ...
    console.error(
      `[entity-operations] getPagesWithEntities failed for document ${documentId}:`,
      e instanceof Error ? e.message : String(e)
    );
    return 0;
  }
...
export function getSegmentStats(
  ...
  try {
    ...
    console.error(
      `[entity-operations] getSegmentStats failed for document ${documentId}:`,
      e instanceof Error ? e.message : String(e)
    );
    return null;
  }
 ...
 */
export function getKGLinkedEntityCount(db: Database.Database, documentId: string): number {
  try {
    return (
      db
        .prepare(
    ...
    console.error(
      `[entity-operations] getKGLinkedEntityCount failed for document ${documentId}:`,
      e instanceof Error ? e.message : String(e)
    );
    return 0;
  }
