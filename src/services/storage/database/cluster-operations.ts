 ...
 */
export function getDocumentEntityNodeIds(db: Database.Database, documentId: string): Set<string> {
  const rows = db
    .prepare(
      `
    SELECT DISTINCT nel.node_id
 ...
 */
function getClusterNodeIds(db: Database.Database, clusterId: string, runId: string): Set<string> {
  const rows = db
    .prepare(
      `
...
export function reassignDocumentToCluster(
  ...
  const clusterCount = (db.prepare('SELECT COUNT(*) as cnt FROM clusters').get() as { cnt: number })
    .cnt;
  if (clusterCount === 0) {
  ...
  const runRow = db
    .prepare('SELECT DISTINCT run_id FROM document_clusters ORDER BY ROWID DESC LIMIT 1')
  ...
  if (bestClusterId && bestOverlap > REASSIGNMENT_JACCARD_THRESHOLD) {
    if (existing) {
      db.prepare('DELETE FROM document_clusters WHERE document_id = ? AND run_id = ?').run(
        documentId,
        runRow.run_id
      );
    }
