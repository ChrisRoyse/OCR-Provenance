 ...
 * @returns OR-joined expanded query string
 */
export function expandQueryWithCoMentioned(
  query: string,
  ...
  for (const nodeId of matchedNodeIds.slice(0, 3)) {
    try {
      ...
      `
        ...
        .all(nodeId, nodeId, nodeId, maxExpansions) as Array<{
      ...
      console.error(
        '[query-expander] expandQueryWithCoMentioned edge query failed for node',
        nodeId,
        ':',
...
export function expandQueryTextForSemantic(query: string, db: Database.Database): string {
  ...
  for (const nodeId of matchedNodeIds.slice(0, 10)) {
    try {
      const row = db
        .prepare('SELECT canonical_name, aliases FROM knowledge_nodes WHERE id = ?')
      ...
      if (row) {
        ...
        if (row.aliases) {
          try {
            ...
            console.error(
              '[query-expander] expandQueryTextForSemantic aliases parse failed for node',
              nodeId,
              ':',
              e instanceof Error ? e.message : String(e)
            );
      ...
      console.error(
        '[query-expander] expandQueryTextForSemantic node lookup failed for node',
        nodeId,
        ':',
        e instanceof Error ? e.message : String(e)
      );
...
export function findMatchingNodeIds(query: string, db: Database.Database): string[] {
  ...
  try {
    ...
    console.error(
      '[query-expander] findMatchingNodeIds FTS5 query failed:',
      e instanceof Error ? e.message : String(e)
    );
  }
...
export function findQuerySuggestions(
  ...
  for (const word of words) {
    try {
      ...
      console.error(
        '[query-expander] findQuerySuggestions FTS5 prefix search failed for word',
        word,
        ':',
        e instanceof Error ? e.message : String(e)
      );
...
function collectKGAliases(db: Database.Database, ftsQuery: string, collected: Set<string>): void {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (row.aliases) {
        try {
          ...
          console.error(
            '[query-expander] collectKGAliases aliases parse failed:',
            e instanceof Error ? e.message : String(e)
          );
        }
      }
    ...
    console.error(
      '[query-expander] collectKGAliases FTS5 query failed:',
      e instanceof Error ? e.message : String(e)
    );
  }
}
