...
function buildKGResolutionMap(
  ...
  try {
    ...
    console.error(
      '[diff-service] buildKGResolutionMap KG table query failed:',
...
export function generateSummary(
  ...
  if (entityDiff && Object.keys(entityDiff.by_type).length > 0) {
    ...
    parts.push(
      `Entities: ${totalCommon} common, ${totalUnique1} unique to doc1, ${totalUnique2} unique to doc2.`
    );
  }
...
export function detectKGContradictions(
  ...
  const entityToNode = new Map<
    string,
    { nodeId: string; canonicalName: string; entityType: string }
  >();
  ...
  try {
    ...
    for (const entity of allEntities) {
      ...
      const row = stmt.get(entity.id) as
        | { entity_id: string; node_id: string; canonical_name: string; entity_type: string }
        | undefined;
      if (row) {
    ...
    console.error(
      '[diff-service] detectKGContradictions KG table query failed:',
      e instanceof Error ? e.message : String(e)
    );
    return emptyResult;
  }
  ...
  const getSemanticEdgesForNode = (nodeId: string): EntityRelationship[] => {
    ...
    for (const edge of edges) {
      ...
      try {
        ...
        console.error(
          '[diff-service] detectKGContradictions edge document_ids parse failed for edge',
          edge.id,
          ':',
          e instanceof Error ? e.message : String(e)
        );
  ...
  for (const nodeId of sharedNodeIds) {
    ...
    for (const relType of doc1RelTypes) {
      if (!doc2RelTypes.has(relType) && doc2RelTypes.size > 0) {
        const rel = allRels.find(
          (r) => r.relationshipType === relType && r.edgeDocumentIds.includes(doc1Id)
        );
        if (!rel) continue;
    ...
    for (const relType of doc2RelTypes) {
      if (!doc1RelTypes.has(relType) && doc1RelTypes.size > 0) {
        const rel = allRels.find(
          (r) => r.relationshipType === relType && r.edgeDocumentIds.includes(doc2Id)
        );
        if (!rel) continue;
