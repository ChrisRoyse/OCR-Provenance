...
import { insertEntity, insertEntityMention } from '../storage/database/entity-operations.js';
import {
...
export async function extractEntitiesFromVLM(
  ...
  if (descriptions.length === 0) {
    throw new Error(
      `No VLM descriptions found for document ${documentId}. Process images with VLM first.`
    );
  }
  ...
  for (const desc of descriptions) {
    ...
    try {
      ...
      console.error(
        `[vlm-entity-extractor] Gemini extraction failed for image ${desc.image_id}: ${(error as Error).message}`
      );
      continue;
    }
    ...
    for (const entity of allEntities) {
      entity.confidence = Math.min(0.85, Math.max(0.5, entity.confidence));
    }

    // Get page number from the image record
    const image = db.prepare('SELECT page_number FROM images WHERE id = ?').get(desc.image_id) as
