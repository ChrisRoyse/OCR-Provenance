 ...
 */
function subBlockEntities(exactGroupList: Entity[][]): Map<string, Set<number>> {
...
function buildNode(groupEntities: Entity[], provenanceId: string): KnowledgeNode {
  ...
  // Average confidence
  const avgConfidence =
    groupEntities.reduce((sum, e) => sum + e.confidence, 0) / groupEntities.length;
