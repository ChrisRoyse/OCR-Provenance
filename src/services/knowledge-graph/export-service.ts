...
function getFilteredData(
  ...
  if (options.entity_type_filter && options.entity_type_filter.length > 0) {
    nodeConditions.push(`entity_type IN (${options.entity_type_filter.map(() => '?').join(',')})`);
  ...
  if (options.relationship_type_filter && options.relationship_type_filter.length > 0) {
...
export function exportGraphML(
  ...
  xml += '  <key id="entity_type" for="node" attr.name="entity_type" attr.type="string"/>\n';
  xml += '  <key id="canonical_name" for="node" attr.name="canonical_name" attr.type="string"/>\n';
  xml += '  <key id="document_count" for="node" attr.name="document_count" attr.type="int"/>\n';
  xml += '  <key id="mention_count" for="node" attr.name="mention_count" attr.type="int"/>\n';
  xml += '  <key id="avg_confidence" for="node" attr.name="avg_confidence" attr.type="double"/>\n';
  xml += '  <key id="edge_count" for="node" attr.name="edge_count" attr.type="int"/>\n';
  ...
  xml += '  <key id="evidence_count" for="edge" attr.name="evidence_count" attr.type="int"/>\n';

  if (options.include_metadata) {
    xml += '  <key id="metadata" for="node" attr.name="metadata" attr.type="string"/>\n';
    xml += '  <key id="edge_metadata" for="edge" attr.name="metadata" attr.type="string"/>\n';
  }
  ...
  const jsonLd = {
    ...
    '@graph': [
      ...nodes.map((node) => ({
        ...
        ...(options.include_metadata && node.metadata ? { metadata: node.metadata } : {}),
      })),
      ...edges.map((edge) => ({
        '@id': `kg:edge/${edge.id}`,
